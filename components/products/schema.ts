...
const imageSchema = z.union([
  z.any() // para o File antes do upload
    .refine((file: File) => file.size <= MAX_FILE_SIZE, `Max 5MB`)
    .refine((file: File) => ACCEPTED_IMAGE_TYPES.includes(file.type), "Formato inválido"),
...
export const docSchema = z.object({
  ...
  price: z
    ...
    .transform((val) => {
      ...
      return (Number.parseInt(numericValue) / 100).toFixed(2);
    }),
  ...
  images: z.array(imageSchema).min(1, "Selecione ao menos uma imagem"),
   description: z.string().min(1, { message: "Por favor selecione um status" }),
  docs: docs.optional(),
});
